...
/// ABI note (IPSable): the sable 1.2.2 jar declares `dispose()` but the shipped binary has
/// no such symbol and nothing calls it. Provide a no-op so the declaration always binds.
#[unsafe(no_mangle)]
