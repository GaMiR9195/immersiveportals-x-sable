...
impl PhysicsHooks for SablePhysicsHooks {
    fn modify_solver_contacts(&self, context: &mut ContactModificationContext) {
        ...
        // hook (every sub-level collider carries MODIFY_SOLVER_CONTACTS), independent of
        // the NEEDS_HOOKS block-property gate below. Gated by env var for A/B isolation:
        // IPL_DISABLE_CLIP=1 turns the pass off without changing anything else.
        if !ipl_clip_disabled() {
            ipl_clip_contacts(context);
...
/// Drop solver contacts that fall inside either collider's clip regions (IPSable,
/// spec §2.5). Contacts are world/scene-frame points; regions are set per body via
/// `Java_ipl_sable_natives_IplRapierNatives_setClipRegions`.
fn ipl_clip_contacts(context: &mut ContactModificationContext) {
    for handle in [context.collider1, context.collider2] {
        ...
        let scene = get_scene_ref(level_collider.scene_id);
        let Some(info) = scene.level_colliders.get(&(id as LevelColliderID)) else {
            continue;
        };
