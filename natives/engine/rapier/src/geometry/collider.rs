...
impl Collider {
    ...
    /// IPL atlas: this collider's portal prefix, if it is an image collider
    /// (see [`ColliderParent::portal_prefix`]).
    pub fn portal_prefix(&self) -> Option<&Pose> {
        self.parent.as_ref().and_then(|p| p.portal_prefix.as_ref())
    }

    /// IPL atlas: set or clear this collider's portal prefix `P` — its effective
    /// world pose becomes `P × bodyPose × pos_wrt_parent`. Does nothing for
    /// standalone colliders. Marks the parent linkage changed so the position is
    /// recomposed and broad/narrow phases pick it up this step.
    pub fn set_portal_prefix(&mut self, prefix: Option<Pose>) {
        if let Some(parent) = self.parent.as_mut() {
            self.changes.insert(ColliderChanges::PARENT);
            parent.portal_prefix = prefix;
        }
    }
    ...
    pub fn compute_broad_phase_aabb(
        ...
        let next_pose = self.parent.and_then(|p| {
            ...
            (parent.soft_ccd_prediction() > 0.0).then(|| {
                p.collider_pose(&parent.predict_position_using_velocity_and_forces_with_max_dist(
                    params.dt,
