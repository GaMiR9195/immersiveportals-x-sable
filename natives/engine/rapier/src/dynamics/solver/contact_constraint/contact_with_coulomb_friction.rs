...
pub(crate) struct ContactWithCoulombFrictionBuilder {
    ...
    /// IPL atlas Tier 1: per-lane portal translations of the two colliders
    /// (ZERO for un-imaged lanes). Local anchors are stored in the MAPPED frame
    /// (`point − shift`); `update` re-adds the shifts so `p1 − p2` is exact.
    portal_shift1: <SimdReal as ScalarType>::Vector,
    portal_shift2: <SimdReal as ScalarType>::Vector,
}
...
impl ContactWithCoulombFrictionBuilder {
    pub fn generate(
        ...
        // IPL atlas Tier 1: image colliders act on their parent body through a
        // translation-only portal — lever arms use the MAPPED center of mass
        // (world_com + portal_shift). Shifts are ZERO for ordinary contacts.
        ...
        let portal_shift1 = <SimdReal as ScalarType>::Vector::from(gather![|ii| manifolds[ii]
            ...
            .into()]);
        #[cfg(not(feature = "simd-is-enabled"))]
        let portal_shift1 = manifolds[0].data.portal_shift1;
        #[cfg(feature = "simd-is-enabled")]
        let portal_shift2 = <SimdReal as ScalarType>::Vector::from(gather![|ii| manifolds[ii]
            .data
            .portal_shift2
            .into()]);
        #[cfg(not(feature = "simd-is-enabled"))]
        ...
        for k in 0..num_points {
            ...
            // Builder. IPL atlas: anchors are stored in the MAPPED frame (point −
            // shift) so a body rotation between substeps doesn't swing the huge
            // portal offset; `update` re-adds the shifts before differencing.
            out_builder.infos[k].local_p1 =
                poses1.inverse_transform_point(solver_contact.point - portal_shift1);
            out_builder.infos[k].local_p2 =
                poses2.inverse_transform_point(solver_contact.point - portal_shift2);
    ...
    pub fn update(
            ...
            .zip(tangent_parts.iter_mut())
        {
            ...
            // IPL atlas: re-add the portal shifts (anchors are stored mapped).
            let p1 = poses1.transform_point(info.local_p1)
                + self.portal_shift1
                + info.tangent_vel * solved_dt;
            let p2 = poses2.transform_point(info.local_p2) + self.portal_shift2;
