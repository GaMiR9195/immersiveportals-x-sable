...
impl ContactWithTwistFrictionBuilder<SimdReal> {
    pub fn generate(
        ...
        let mut friction_center: <SimdReal as ScalarType>::Vector = Default::default();
        ...
        for k in 0..num_points {
            ...
            // Builder. IPL atlas: anchors stored in the MAPPED frame; update()
            // re-adds the shifts (see coulomb builder).
        ...
        out_builder.local_friction_center1 =
            poses1.inverse_transform_point(friction_center - portal_shift1);
        out_builder.local_friction_center2 =
            poses2.inverse_transform_point(friction_center - portal_shift2);
    ...
    pub fn update(
        ...
        for (info, normal_part) in all_infos.iter().zip(normal_parts.iter_mut()) {
            ...
            let p1 = poses1.transform_point(info.local_p1) + self.portal_shift1 + tangent_delta;
        ...
        // tangent parts.
        {
            let p1 = poses1.transform_point(self.local_friction_center1)
                + self.portal_shift1
                + tangent_delta;
            let p2 = poses2.transform_point(self.local_friction_center2) + self.portal_shift2;
