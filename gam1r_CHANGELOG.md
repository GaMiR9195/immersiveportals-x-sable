# Sable-Mod Compatibility: One Root, One Fix

## Symptom

Every mod built on Sable misbehaved when this mod was loaded — treated as one bug:

- Offroad wheels did not move at all.
- Simulated's physics assembler spawned constructions ~0.5 blocks high (more for
  larger structures).
- Simulated ropes were invisible; springs flickered with camera position.
- Aeronautics block entities (swivel bearing, throttle lever) rendered with
  missing parts.

## Root Cause

Stock Sable has NO separate dimension anywhere: a sub-level's container is a field
on the Level instance itself, and every API surface third-party mods touch —
`be.getLevel()`, `SubLevelContainer.getContainer(level)`, `Sable.HELPER.getContaining`,
`WorldAttached` registries, `RigidBodyHandle.of`, packet handlers via
`context.level()`, all of Sable's own render hooks — resolves by Level OBJECT
IDENTITY. This mod hosts plots in `ipl_sable:sublevels`, splitting that identity:
hosted content answered "the void dimension" wherever mod code asked "which world
am I in", while adjacent code (renderers keyed to `Minecraft.level`, physics scenes
owned by the parent dimension) answered "the parent". Every symptom is one face of
that split. Earlier fixes patched single faces (wheel-mount drain filter, per-method
assembler redirects, one hand-picked ThreadLocal arming site) instead of the split
itself.

## Implemented Change

**Client: the void dimension no longer exists.** All sub-level sync is stamped per
packet with the owning ship's PARENT dimension. `PacketRedirection.createRedirectedMessage`
gained a resolver hook (`IplHostedPacketRouting`): vanilla chunk/light/block/BE
packets resolve by plot coordinates, Sable tracking payloads by plot slot or UUID,
batched movement/interpolation payloads go unstamped and are routed per sub-level id
on the client. Result: a ship's plot chunks, block entities and tracking state live
in the viewed `ClientLevel`'s container — the stock Sable client model — so every
mod's client code (rope registries, spring renderers with their
`getContainer(minecraft.level)` lookups, render bounding boxes, wheel visuals,
aeronautics BE renderers) runs unmodified against the world it expects. The mirror
render mixins that re-drew hosted ships from the void container are deleted
(`IplHostedSubLevelRenderMixin` now draws only straddle projections; Sable's own
hooks draw ships natively, vanilla and Sodium main pass alike).

Supporting client structure:

- `IplClientSubLevelLookupBridgeMixin`: sub-level lookups fall through across
  client levels on a miss (unstamped/UDP packets, flip races).
- `IplSnapshotInfoFanoutMixin`: the per-player interpolation clock feeds every
  client level's container, so portal-visible ships keep interpolating.
- Parent flips adopt the EXISTING client object into the destination level's
  container (`IplClientAdopt` + container surgery duck), buffered interpolation
  timeline mapped through the exact portal transform as before — compiled render
  data survives, no visual gap. A follow-up full re-sync (`IplTrackingResyncDuck`)
  rebinds chunk/BE objects and light to the destination level; the start-tracking
  dedupe turns it into an adopt/skip instead of a duplicate allocation.
- `IplPredictionBridgeMixin` now fans out confirmations for any plot-range
  position rather than gating on the removed hosting client level.

**Server: routing is structural, not per-call-site.** The world-frame context is
armed by the fused step itself around EVERY per-sub-level physics phase
(`updateMergedMassData`, the `sable$physicsTick` actor pass, `applyQueuedForces`)
for every hosted ship — wheels' suspension casts, lift providers and any mod's
actors get parent-frame terrain with no per-mod arming mixin. The world-frame
router additionally routes `getChunk` (chunk-grid gate) and `getEntities*`
(AABB gate), which makes Simulated's assembler disassembly work through the
ordinary armed BE tick. Physics objects (ropes, boxes) created by hosted content
are routed to the parent dimension's pipeline — the scene that owns the ship's
body and terrain — via the ownership guard, so rope/ship/terrain collision shares
one chart again.

**Removed point patches** (superseded by the structural fixes):

- `IplWheelMountLevelFilterMixin` + `IplWheelMountInvoker` — under the fused step
  there is exactly one native step per substep, so Offroad's stock global drain
  is timing-correct, and per-body calls already forward to the owning pipeline.
- `IplHostedPhysicsTickContextMixin` — arming folded into the fused step driver.
- `IplAssemblerDisassemblyParentMixin` — covered by the router's
  `getChunk`/`setBlock` routing under the armed BE tick. Its portal-restore side
  effect moved to Sable's own `SubLevelAssemblyHelper.moveBlocks` choke point
  (`IplDisassemblyPortalRestoreMixin` + `IplAssemblyTransformAccessor`), covering
  every disassembly path, not just Simulated's.
- `IplHostedRenderDataMixin` and the hosted-mirror halves of the render mixins.
- The parent-stamp RPC (`setParent`) — parent identity is now the container's
  level itself.

Rapier natives were audited against stock (`sable_rapier` sources) and needed no
change for this bug class: body ids are world-global under the atlas and the
chart machinery was already frame-correct; the incompatibility lived entirely in
Java-side Level-identity resolution.

Known edges: disassembly build-height pre-checks run against the hosting profile
(-64..320), which is a superset of nether/end — actual placement is enforced
per-block by the parent level. Assembly's reported half-block offset could not be
reproduced in the pose math (verified world-invariant end to end); it is expected
to have been a client-frame artifact of the removed void-level render path —
retest after this change.

## Files

- `src/main/java/qouteall/imm_ptl/core/network/PacketRedirection.java`
- `src/main/java/ipl/sable/dim/IplHostedPacketRouting.java` (new)
- `src/main/java/ipl/sable/mixin/client/IplClientSubLevelLookupBridgeMixin.java` (new)
- `src/main/java/ipl/sable/mixin/client/IplSnapshotInfoFanoutMixin.java` (new)
- `src/main/java/ipl/sable/mixin/client/IplClientContainerAdoptMixin.java` (new)
- `src/main/java/ipl/sable/mixin/client/IplSubLevelLevelAccessor.java` (new)
- `src/main/java/ipl/sable/mixin/client/IplLevelPlotContainerAccessor.java` (new)
- `src/main/java/ipl/sable/client/IplClientAdopt.java` (new)
- `src/main/java/ipl/sable/client/IplClientLookupBridge.java` (new)
- `src/main/java/ipl/sable/duck/IplClientContainerAdoptDuck.java` (new)
- `src/main/java/ipl/sable/duck/IplTrackingResyncDuck.java` (new)
- `src/main/java/ipl/sable/mixin/IplDisassemblyPortalRestoreMixin.java` (new)
- `src/main/java/ipl/sable/mixin/IplAssemblyTransformAccessor.java` (new)
- `src/main/java/ipl/sable/client/IplParentDimSync.java` (rewritten)
- `src/main/java/ipl/sable/client/IplClientHostedLookup.java`
- `src/main/java/ipl/sable/client/IplStaffPortalBeamRenderer.java`
- `src/main/java/ipl/sable/client/IplStraddleStaffPick.java`
- `src/main/java/ipl/sable/mixin/SableStartTrackingDedupeMixin.java`
- `src/main/java/ipl/sable/mixin/SableCrossDimTrackingMixin.java`
...
- `src/main/java/ipl/sable/mixin/client/IplPredictionBridgeMixin.java`
- `src/main/java/ipl/sable/atlas/IplFusedStep.java`
- `src/main/java/ipl/sable/transit/SableRehomeOps.java`
- `src/main/resources/ipl_sable.mixins.json`
- Deleted: `IplWheelMountLevelFilterMixin`, `IplWheelMountInvoker`,
  `IplHostedPhysicsTickContextMixin`, `IplAssemblerDisassemblyParentMixin`,
  `IplHostedRenderDataMixin`

## Verification Needed

Runtime-check with Offroad + Simulated + Aeronautics: assemble a single block
(no vertical offset), drive wheels on terrain (grip and visual spin), ropes and
springs visible from all camera angles, swivel bearing split and throttle lever
fully rendered, block breaking/placing on ships, hosted transit through portals
(seamless, ship re-lights in the destination), Sodium portal views.

No build or automated check was run for these changes (per request).
